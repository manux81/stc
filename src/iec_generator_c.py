...
class CCodeGenerator(NodeVisitor):
    ...
    def visit_library(self, node):
        ...
        emitted_runtime_function = False
        for name, runtime_source in C_RUNTIME_FUNCTIONS.items():
            if self._contains_function(node, {name}):
                if not emitted_runtime_function:
                    self.text += "/* Runtime functions referenced by the translated program. */\n"
                    emitted_runtime_function = True
    ...
    def _standard_runtime_prelude():
        return r'''/* Common state layout used by the standard function-block placeholders. */
typedef struct TON {
    bool Q;
    bool Q1;
    int64_t ET;
...
/* Standard IEC functions mapped to portable C operations. */
#define ABS(x) fabs(x)
...
static inline const char *INT_TO_STRING(int16_t value)
{
...
static inline int16_t STRING_TO_INT(const char *value)
{
    return (int16_t)strtol(value, NULL, 10);
}

static inline const char *CONCAT(const char *left, const char *right)
{
...
static inline const char *INSERT(const char *text, const char *value, int position)
{
    (void)value;
    (void)position;
    return text;
...
static inline const char *DELETE(const char *text, int length, int position)
{
    (void)length;
    (void)position;
    return text;
...
static inline const char *REPLACE(const char *text, const char *value, int length, int position)
{
    (void)value;
    (void)length;
    (void)position;
...
static inline int16_t FIND(const char *text, const char *value)
{
    const char *match = strstr(text, value);
    return match ? (int16_t)(match - text) : -1;
}

static inline const char *LEFT(const char *text, int length)
{
    (void)length;
    return text;
}

static inline const char *RIGHT(const char *text, int length)
{
    (void)length;
    return text;
}

static inline const char *MID(const char *text, int length, int position)
{
    (void)length;
    (void)position;
    return text;
